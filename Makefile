...
VPATH += $(CRAZYFLIE_BASE)/vendor/keras2c
VPATH += $(CRAZYFLIE_BASE)/vendor/keras2c/include
...
INCLUDES += -I$(CRAZYFLIE_BASE)/vendor/keras2c
INCLUDES += -I$(CRAZYFLIE_BASE)/vendor/keras2c/include


# Compiler flag to enable the 16 Bit floating point format in alternative mode
...
all: venv/ bin/ bin/dep bin/vendor check_submodules build
build:
# Each target is in a different line, so they are executed one after the other even when the processor has multiple cores (when the -j option for the make command is > 1). See: https://www.gnu.org/software/make/manual/html_node/Parallel.html
	...
	@cd $(CRAZYFLIE_BASE)/vendor/keras2c; \
		...
		$(PYTHON) -m keras2c --dtype _Float16 -m $(CRAZYFLIE_BASE)/models/forward.h5 neural_forward; \
		$(PYTHON) -m keras2c --dtype _Float16 -m $(CRAZYFLIE_BASE)/models/recurrent.h5 neural_recurrent; \
		$(PYTHON) -m keras2c --dtype _Float16 -m $(CRAZYFLIE_BASE)/models/cascaded.h5 neural_cascaded; 
...
venv/:
	@$(PYTHON) -m venv venv
	@. $(CRAZYFLIE_BASE)/venv/bin/activate; \
		$(PYTHON) -m pip install -r $(CRAZYFLIE_BASE)/vendor/keras2c/requirements.txt
