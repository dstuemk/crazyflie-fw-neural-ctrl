...
void kalmanCoreExternalizeState(const kalmanCoreData_t* this, state_t *state, const Axis3f *acc, const Axis3f *gyro, const Axis3f *gyroUnfiltered, uint32_t tick)
{
  ...
  state->flat_rotation_matrix = (flat_rotation_t){
    .timestamp = tick,
    .a = this->R[0][0],
    .b = this->R[0][1],
    .c = this->R[0][2],
  ...
  // New in Project Phoenix: Append rpy_dot information from Gyro to state
  state->attitude_rate = (rpy_dot_t){
    ...
    state->attitude_rate_unfiltered = (rpy_dot_t){
    .timestamp = tick,
    .x = gyroUnfiltered->x,  // [deg/s]
    .y = gyroUnfiltered->y,  // [deg/s]
    .z = gyroUnfiltered->z  // [deg/s]
  };
